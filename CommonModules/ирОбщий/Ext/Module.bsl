...
Функция ПолучитьПараметрыЗапускаПриложения1СТекущейБазыЛкс(Знач ИмяПользователяИнфобазы = "", Знач ПарольПользователяИнфобазы = "", КодРазрешения = "", РежимКонфигуратора = Ложь,
	...
	СтрокаСоединения = "", ОткрытьПортативныеИнструменты = Ложь, РежимИнтерфейсаТакси = Ложь) Экспорт 
...
//  Ложь         – иначе.
//
Функция ЛиКорневойТипСсылкиЛкс(Знач КорневойТип) Экспорт
...
//
Функция ПолучитьРасширенноеПредставлениеЗначенияЛкс(Значение, КолонкаТабличногоПоля = Неопределено, ДобавлятьПредставлениеТипа = Ложь, РасширенноеПредставлениеХранилищЗначений = Ложь) Экспорт
...
Функция ПолучитьСтрокуПутиВДеревеЛкс(СтрокаДерева, ИмяКолонки = "Имя", ИгнорироватьПростойПервыйУровень = Ложь) Экспорт
...
Функция ПолучитьМассивПутиСтрокиДереваЛкс(СтрокаДерева, ИмяКолонки = "Имя") Экспорт

	Если СтрокаДерева.Родитель = Неопределено Тогда 
		Результат = Новый Массив;
	Иначе
		Результат = ПолучитьМассивПутиСтрокиДереваЛкс(СтрокаДерева.Родитель, ИмяКолонки);
	КонецЕсли;
...
Функция ВыбратьКаталогВФормеЛкс(Каталог, ФормаДляУстановкиМодифицированности = Неопределено) Экспорт
...
Функция ИнтерактивноЗаписатьВЭлементУправленияЛкс(ЭлементУправления, Значение, ФормаИнициатор = Неопределено) Экспорт
...
// Результат - Структура
Функция ПолучитьКлючНабораЗаписейИзКлючаЗаписиЛкс(КлючЗаписи) Экспорт 
...
Функция ПолучитьТекстЗапросаПоВыбраннымТаблицамЛкс(МассивИменОбъектовМД, ВариантИсточников = 0, ПервыеNКаждойТаблицы = 0, ПодключатьПоляКоличестваДвижений = Ложь) Экспорт
	...
	Для Каждого ИмяОбъектаМД Из МассивИменОбъектовМД Цикл
		...
		КонецЕсли;
		ТекстЗапроса = ТекстЗапроса + Сред(ТекстНеобязательныхПолей, 2) + ТекстОбщихМетаПолей + ", """ + ИмяТаблицы + """ КАК _ПолноеИмяТаблицы ИЗ " + ИмяТаблицы + " КАК Т"
			+ ТекстУсловияСоединения;
	КонецЦикла;
...
Функция ЗапуститьСеансПодПользователемЛкс(ИмяПользователяИнфобазы, ПарольПользователяИнфобазы, ТипCOMОбъекта = "", РежимЗапуска = "Авто", КодРазрешения = "",
	...
	РежимИнтерфейсаТакси = Ложь, Элевация = Ложь) Экспорт 
	...
	Иначе
		...
		Иначе
			...
			Если ИмяПараметра = Неопределено Тогда
				ИмяПараметра = "" + ПолучитьРасширенноеПредставлениеЗначенияЛкс(ЗначениеПараметра);
			КонецЕсли; 
...
Функция ПолучитьОбъектПоПолномуИмениМетаданныхЛкс(ПолноеИмяМД) Экспорт
	...
	Иначе
		...
		Если Истина
			...
			И ИмяОбъекта = "ирКлсПолеТекстовогоДокументаСКонтекстнойПодсказкой"
		Тогда
...
Функция ОткрытьФормуПроизвольногоЗначенияЛкс(РасширенноеЗначение, РедактированиеРазрешено = Истина, СтандартнаяОбработка = Истина,
	ЗаголовокФормы = "", РедактироватьМодально = Истина, ПринудительноВОтдельнойФорме = Истина) Экспорт 
	...
	Тогда
		...
		Если Ложь
			...
			Или ТипРасширенногоЗначения = Тип("ДеревоЗначений") 
		Тогда
			ФормаРедактирования = ирКэш.Получить().ПолучитьФорму("ТаблицаЗначений", , РасширенноеЗначение);
		ИначеЕсли ТипРасширенногоЗначения = Тип("МоментВремени") Тогда
			ФормаРедактирования = ирКэш.Получить().ПолучитьФорму("МоментВремени", , РасширенноеЗначение);
		ИначеЕсли ТипРасширенногоЗначения = Тип("ТабличныйДокумент") Тогда
			ФормаРедактирования = ирКэш.Получить().ПолучитьФорму("ТабличныйДокумент", , РасширенноеЗначение);
		ИначеЕсли ТипРасширенногоЗначения = Тип("Граница") Тогда
			ФормаРедактирования = ирКэш.Получить().ПолучитьФорму("Граница", , РасширенноеЗначение);
		ИначеЕсли ТипРасширенногоЗначения = Тип("Массив") Тогда
			ФормаРедактирования = ирКэш.Получить().ПолучитьФорму("Массив", , РасширенноеЗначение);
		ИначеЕсли ТипРасширенногоЗначения = Тип("УникальныйИдентификатор") Тогда
			ФормаРедактирования = ирКэш.Получить().ПолучитьФорму("УникальныйИдентификатор", , РасширенноеЗначение);
		ИначеЕсли ТипРасширенногоЗначения = Тип("СписокЗначений") Тогда
			ФормаРедактирования = ирКэш.Получить().ПолучитьФорму("СписокЗначений", , РасширенноеЗначение);
		ИначеЕсли ТипРасширенногоЗначения = Тип("Строка") Тогда
			ФормаРедактирования = ирКэш.Получить().ПолучитьФорму("Текст", , Новый УникальныйИдентификатор());
		ИначеЕсли ТипРасширенногоЗначения = Тип("Тип") Тогда
			ФормаРедактирования = ирКэш.Получить().ПолучитьФорму("ВыборРедактируемыхТипов", , Новый УникальныйИдентификатор());
			ФормаРедактирования.МножественныйВыбор = Ложь;
		ИначеЕсли ТипРасширенногоЗначения = Тип("ОписаниеТипов") Тогда
			ФормаРедактирования = ирКэш.Получить().ПолучитьФорму("ВыборРедактируемыхТипов", , РасширенноеЗначение);
...
Функция ПолеВводаКолонкиРасширенногоЗначения_НачалоВыбораЛкс(ТабличноеПоле, СтандартнаяОбработка, РасширенноеЗначение = Null, ИспользоватьОграничениеТипа = Ложь,
	...
	Если РасширенноеЗначение = Неопределено Тогда
		...
		КонецЕсли;
	Иначе
		ЗначениеИзменено = ЯчейкаТабличногоПоляРасширенногоЗначения_ВыборЛкс(ТабличноеПоле, СтандартнаяОбработка, РасширенноеЗначение, Истина, Истина, Данные);
	...
	Для Каждого Колонка Из Элемент.Колонки Цикл
		...
		Если Формат(ЗначениеЯчейки, Колонка.Формат) = Ячейка.Текст Тогда // Здесь могут быть обращения к БД
			...
			Если ТипЗнч(ЗначениеЯчейки) <> Тип("Строка") Тогда
				ПредставлениеЗначения = ПолучитьРасширенноеПредставлениеЗначенияЛкс(ЗначениеЯчейки, Колонка,, РасширенноеПредставлениеХранилищЗначений);
			КонецЕсли; 
...
Функция ОткрытьОбъектВРедактореОбъектаБДЛкс(ОбъектБД, ИскомоеЗначение = Неопределено) Экспорт
	
	РедакторОбъектаБД = ПолучитьОбъектПоПолномуИмениМетаданныхЛкс("Обработка.ирРедакторОбъектаБД");
	#Если Сервер И Не Сервер Тогда
	    РедакторОбъектаБД = Обработки.ирРедакторОбъектаБД.Создать();
	#КонецЕсли
	Форма = РедакторОбъектаБД.РедактироватьМодифицированныйОбъект(ОбъектБД, ИскомоеЗначение);
	Возврат Форма;
...
Процедура ПрименитьСтрокуПоискаКТабличномуПолюДереваЛкс(ТабличноеПолеДерева, СтрокаПоиска, ИменаКолонокДанныхДляПоиска, выхСтруктураПоиска) Экспорт 
...
Процедура СкопироватьФайлыЛкс(Источник, Приемник) Экспорт
	
	Файлы = НайтиФайлы(Источник, "*.*");
	Для Каждого Файл Из Файлы Цикл
		ФайлПриемник = Новый Файл(Приемник + "\" + Файл.Имя);
		Если Файл.ЭтоКаталог() Тогда
...
Функция ПолучитьСтрокуФильтраДляВыбораФайлаЛкс(СтрокаРасширений, ОписаниеФормата = "", РазрешитьВсеФайлы = Истина) Экспорт
	...
	Расширения = ирОбщий.ПолучитьМассивИзСтрокиСРазделителемЛкс(СтрокаРасширений,",");
	Результат = "";
...
Функция ПолучитьСтруктуруХраненияБДЛкс(Знач ОтборПоМетаданным = Неопределено, ЛиИменаБД = Ложь, ВычислитьИменаИндексов = Истина) Экспорт
			...
			Если ЭлементПорядка <> Неопределено Тогда
				...
				Если Не ПорядокУстановлен Тогда
					...
					Если ОсновнойЭУ.Значение.Порядок.Найти(ПредопределенноеПоле) = Неопределено Тогда 
						...
						КонецЕсли; 
					КонецЕсли; 
					// Обязательную установку делаем, чтобы в шапках появились индикаторы сортировки
...
Процедура НастроитьДобавленныеКолонкиТабличногоПоляЛкс(Знач ТабличноеПоле, ОписанияТиповКолонок = Неопределено, МетаданныеКолонок = Неопределено, ДоступныеПоляВыбора = Неопределено) Экспорт 
